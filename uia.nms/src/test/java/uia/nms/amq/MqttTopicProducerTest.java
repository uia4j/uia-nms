...
public class MqttTopicProducerTest implements NmsMatching {

    private NmsEndPoint endPoint;

    private int index;

    public MqttTopicProducerTest() {
        this.endPoint = new NmsEndPoint(null, null, "tcp://localhost", "61616");
    }

    @Test
    public void testSend() throws Exception {
        MqttTopicSubscriber s = new MqttTopicSubscriber();
        s.start("a/b/c");
        MqttTopicPublisher p = new MqttTopicPublisher();
        p.start();
        p.send("a/b/c", "data", "abcdefgdfadfad", true);
        p.stop();
